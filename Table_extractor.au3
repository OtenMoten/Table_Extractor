...
Func _On_Button()
	...
	Switch @GUI_CtrlId ;Check which button sent the message
		...
		Case $btnKillSpaces
			...
			Local $fileName = FileFindFirstFile("*.ods")
			Local $newFileName = FileFindNextFile($fileName)

			Local $OOobject = _OOoCalc_BookOpen($pathInternalDir &  "\" & $newFileName)

			_OOoCalc_BookSaveAs($OOobject, "C:\Test_Extraktor\Internal\test.xls")
			sleep(500)
...
Func _Start_Tabula_with_file($pathPDFfile)
	...
	While NOT FileExists($newPathCSVfile)
		...
		If(WinActive("Speichern unter") = true) Then
			MsgBox($MB_SYSTEMMODAL, "TEST", "Speichern unter OK WinActive")
			;; Speicher für den Benutzer die Datei ab
		EndIf
